...
async function load(client, cm) {
    cm.register({
        ...
        handler: async (msg) => {
            ...
            msg.channel
                .send(result)
                .catch(err => msg.channel.send(i18n.parse(msg.lang, "basic.say.error.invaildparam")));
        }
    });
    cm.register({
        command: "random",
    ...
    cm.register({
        ...
        handler: async (msg) => {
            ...
            if (args[1].toLowerCase() === "command") {
                ...
                return msg.reply(`${args[2]} is ${bool ? "disabled" : "enabled"}!`);
            }
            msg.reply("not found...");
        }
    });
    ...
    cm.register({
        ...
        handler: async (msg, { prefix }) => {
            ...
            if (banner) {
                ...
                burl = `https://serux.pro/rendercolour?hex=${banner_color?.replace("#", "")}&height=200&width=512`;
                embed.setImage(burl);
            }
            embed.setColor(banner_color);
            embed.setDescription(`Account Created on ${""} | [Avatar](${aurl}) | [Banner](${burl}) | Color: ${banner_color}`);
            //snowflake
